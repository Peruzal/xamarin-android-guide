...
Views have two required properties, `width` and `height`. These two properties are set using the `android:layout_width` and `android:layout_height`.
The view can be set to either `wrap_content`,`match_parent` or a specific size. Use the `match_parent` to take the full width or height as the parent layout.
Use the `wrap_content` to take as much space as the view requires. Another important property is the `ID` property, this is used to name the views so that they can be referenced in code and in other parts of the layout file.
